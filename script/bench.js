...
const wordToBench = "pou";

const searchByInput = () => {
  if (wordToBench.length > 1) {
    const normaliezedText = normalizeText(wordToBench);
    ...
    data.recipes.forEach((recipe) => {
      if (
        ...
        console.log("trouvé " + wordToBench);
      }
    });
  }
};
...
const wordToBench = "po";

const searchByInput = () => {
  if (wordToBench.length > 1) {
    const recipes = data.recipes;
    const j = recipes.length;
    const normaliezedValue = normalizeText(wordToBench);
    ...
    for (let i = 0; i < j; i++) {
      if (normalizeText(recipes[i].name).includes(normaliezedValue)) {
        ...
        console.log(recipes[i] + " trouvé dans les ingrediants");
      }
    }
  }
};
